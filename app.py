...
<div id="manual-collect-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:500;align-items:center;justify-content:center;">
  <div style="background:var(--surface);border-radius:16px;width:720px;max-width:96vw;max-height:92vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(26,60,46,0.22);overflow:hidden;">
    ...
    <div id="mc-pane-text" style="display:none;flex-direction:column;flex:1;overflow:hidden;">
      ...
      <!-- Barre unique : Annuler + Analyser (masqué après analyse) + Sauvegarder (affiché après analyse) -->
      <div style="padding:13px 24px;border-top:1px solid var(--border);display:flex;gap:9px;justify-content:flex-end;flex-shrink:0;align-items:center;">
        <button onclick="closeManualCollect()" style="background:var(--surface2);border:1px solid var(--border);border-radius:7px;padding:8px 16px;font-size:12px;cursor:pointer;font-family:'DM Sans',sans-serif;">Annuler</button>
        ...
        <button id="mc-text-save-btn" onclick="saveTextCollect()" style="display:none;background:#1e8f54;color:#fff;border:none;border-radius:7px;padding:8px 18px;font-size:12px;font-weight:700;cursor:pointer;font-family:'DM Sans',sans-serif;">💾 Sauvegarder le dispositif</button>
      </div>
    </div>
...
async function runTextCollect() {
  ...
  var runBtn = document.getElementById('mc-text-run-btn');
  var saveBtn = document.getElementById('mc-text-save-btn');
  runBtn.disabled = true; runBtn.textContent = 'Analyse en cours…';
  ...
  try {
    ...
    // Swap buttons: hide Analyser, show Sauvegarder
    ...
    runBtn.disabled = false; runBtn.textContent = 'Analyser';
  }
}
...
async function saveTextCollect() {
  ...
  try {
    ...
    var saved = await res.json();
    if (saved.status === 'duplicate') {
      showToast('Ce dispositif est deja dans la base !');
      btn.disabled = false; btn.textContent = 'Sauvegarder le dispositif';
    } else {
      closeManualCollect();
